...
deno <- animal_lactations |>
  ...
  filter(date_left <= date_end | is.na(date_left) 
         & date_fresh <= date_end) |> 
  # filters cows fresh before start date
  filter(date_fresh <= date_start & date_left >= date_start |
           date_fresh <= date_start & is.na(date_left) |
  # filters cows fresh after start date
    date_fresh >= date_start & date_left <= date_end |
           date_fresh >= date_start & is.na(date_left)
         )
...
denominators_2024 <- fxn_create_deno(label = "2022",
